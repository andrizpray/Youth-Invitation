    ...
    <div style={{ fontFamily: '"Amiri", "Playfair Display", serif', color: colors.accent, backgroundColor: colors.secondary }}>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="text-white/50 text-xs uppercase tracking-[0.3em] mb-10">Bismillahirrahmanirrahim</p>
        ...
        <div className="absolute bottom-8 animate-bounce text-white/30 text-xl z-10">↓</div>
      </section>
      ...
      <section className="py-16 px-8 text-center" style={{ backgroundColor: colors.primary + '18' }}>
        ...
        <p className="text-xs opacity-50 uppercase tracking-widest">QS. Ar-Rum: 21</p>
      </section>

      {/* Mempelai */}
      {(invitation.parent_name || invitation.parent_name2) && (
        <section className={sectionClass} style={{ backgroundColor: colors.secondary }}>
          <p className="text-xs uppercase tracking-[0.3em] opacity-40 mb-6">Mempelai</p>
          ...
          <div className="flex flex-col gap-6 max-w-xs w-full">
            {invitation.parent_name && (
              <div className="p-5 rounded-xl text-left" style={{ backgroundColor: colors.primary + '15', border: `1px solid ${colors.primary}33` }}>
                ...
                <p className="text-xs opacity-50 mt-1">Putra dari</p>
                <p className="text-sm opacity-70">{invitation.parent_name}</p>
              </div>
            )}
            {invitation.parent_name2 && (
              <div className="p-5 rounded-xl text-left" style={{ backgroundColor: colors.primary + '15', border: `1px solid ${colors.primary}33` }}>
                <p className="text-xl font-semibold" style={{ color: colors.primary }}>{invitation.partner_name2}</p>
                <p className="text-xs opacity-50 mt-1">Putri dari</p>
                <p className="text-sm opacity-70">{invitation.parent_name2}</p>
              </div>
            )}
          </div>
        </section>
      ...
      {(invitation.date_akad || invitation.time_akad) && (
        <section className={sectionClass} style={{ backgroundColor: colors.primary + '15' }}>
          <p className="text-xs uppercase tracking-[0.3em] opacity-40 mb-4">Akad Nikah</p>
          <div className="w-12 h-px mx-auto mb-8" style={{ backgroundColor: colors.primary }} />
          <p className="text-3xl font-bold mb-3" style={{ color: colors.primary }}>
          ...
          {invitation.time_akad && (
            <p className="text-lg opacity-60">{invitation.time_akad} WIB</p>
          )}
        </section>
      )}
      ...
      {(invitation.date_resepsi || invitation.time_resepsi) && (
        <section className={sectionClass} style={{ backgroundColor: colors.secondary }}>
          <p className="text-xs uppercase tracking-[0.3em] opacity-40 mb-4">Resepsi Pernikahan</p>
          <div className="w-12 h-px mx-auto mb-8" style={{ backgroundColor: colors.primary }} />
          <p className="text-3xl font-bold mb-3" style={{ color: colors.primary }}>
          ...
          {invitation.time_resepsi && (
            <p className="text-lg opacity-60">{invitation.time_resepsi} WIB</p>
          )}
        </section>
      )}
      ...
      {invitation.location && (
        <section className={sectionClass} style={{ backgroundColor: colors.primary + '15' }}>
          <p className="text-xs uppercase tracking-[0.3em] opacity-40 mb-4">Lokasi</p>
          <div className="w-12 h-px mx-auto mb-8" style={{ backgroundColor: colors.primary }} />
          <p className="text-2xl font-semibold mb-2">{invitation.location}</p>
          {invitation.address && (
            <p className="text-sm opacity-60 max-w-xs leading-relaxed mb-8">{invitation.address}</p>
          )}
          {invitation.maps_url && (
            <a
      ...
      {invitation.story && (
        <section className={sectionClass} style={{ backgroundColor: colors.secondary }}>
          <p className="text-xs uppercase tracking-[0.3em] opacity-40 mb-4">Kisah Kami</p>
          <div className="w-12 h-px mx-auto mb-8" style={{ backgroundColor: colors.primary }} />
      ...
      {photos.length > 0 && (
        <section className="py-20 px-6" style={{ backgroundColor: colors.primary + '15' }}>
          <p className="text-xs uppercase tracking-[0.3em] opacity-40 text-center mb-4">Galeri</p>
          <div className="w-12 h-px mx-auto mb-8" style={{ backgroundColor: colors.primary }} />
      ...
      {/* RSVP */}
      <section className={sectionClass} style={{ backgroundColor: colors.secondary }}>
        <p className="text-xs uppercase tracking-[0.3em] opacity-40 mb-4">Konfirmasi Kehadiran</p>
        <div className="w-12 h-px mx-auto mb-8" style={{ backgroundColor: colors.primary }} />
        <RsvpSection
          guests={guests}
      ...
      >
        <p className="text-white/50 text-sm mb-2">Wassalamualaikum Warahmatullahi Wabarakatuh</p>
        ...
        <p className="text-white/30 text-xs mt-8">Terima kasih atas doa dan restu Anda</p>
      </footer>
    </div>
  );
}
