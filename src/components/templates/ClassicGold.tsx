...
export default function ClassicGold({ invitation, guests, onRsvpSubmit, rsvpStatus, rsvpError }: TemplateProps) {
  ...
  return (
    <div style={{ fontFamily: '"Playfair Display", serif', color: colors.accent, backgroundColor: colors.secondary }}>
      ...
      >
        <p className="text-white/60 text-xs uppercase tracking-[0.3em] mb-6">Bismillahirrahmanirrahim</p>
        ...
        <p className="text-white/50 text-sm mb-2">Kami mengundang Bapak/Ibu/Saudara/i</p>
        <p className="text-white/50 text-sm mb-10">pada pernikahan kami</p>
        ...
        <div className="mt-16 animate-bounce text-white/30 text-xl">↓</div>
      </section>

      {/* Mempelai */}
      {(invitation.parent_name || invitation.parent_name2) && (
        <section className={sectionClass} style={{ backgroundColor: colors.secondary }}>
          <p className="text-xs uppercase tracking-[0.3em] opacity-40 mb-4">Putra-Putri dari</p>
          ...
          <div className="space-y-6 max-w-sm">
            {invitation.parent_name && (
              <div>
                ...
                <p className="text-sm opacity-60 mt-1">Putra dari {invitation.parent_name}</p>
              </div>
            )}
            {invitation.parent_name2 && (
              <div>
                <p className="text-lg font-semibold" style={{ color: colors.accent }}>{invitation.partner_name2}</p>
                <p className="text-sm opacity-60 mt-1">Putri dari {invitation.parent_name2}</p>
              </div>
            )}
          </div>
        </section>
      ...
      >
        <p className="text-white/40 text-xs uppercase tracking-widest mb-6">Dengan penuh cinta</p>
        <p
